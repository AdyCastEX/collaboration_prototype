...
class Encoder:
    ...
    def create_generic_operation(self,op_name,obj_name,mode):
        ''' creates a generic encoded operation 
        
        Parameters:
        op_name -- name of the operator
        obj_name -- name of the target object of the operator
    ...
    def resize(self,operator,target_object,mode):
    ...
    def delete(self,operator,target_object,mode):
        '''creates an operation with attributes specific to a delete'''
        if target_object == None:
            op = self.create_generic_operation(operator.name, bpy.context.scene.active_obj_name, mode)
        else:
            op = self.create_generic_operation(operator.name, target_object.name, mode)
        op['use_global'] = operator.properties['use_global']
        return op
    
    def add_generic_object(self,operator,target_object,mode):
